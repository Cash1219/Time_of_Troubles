...
price={
 ...
 'rubber':50,'dye':35,'gold':80,'lead':35,'oil':40,'omocha':0
}
for m in re.finditer(r'(?ms)^\s*(pm_goods_[A-Za-z0-9_]+)\s*=\s*\{',t):
    ...
    print(name)
    print('  in :', ', '.join(f'{g}*{v}' for g,v in ins) or '-')
    print('  out:', ', '.join(f'{g}*{v}' for g,v in outs) or '-')
    print(f'  val: in={iv:.0f}, out={ov:.0f}, net={net:.0f}, roi={"-" if roi is None else round(roi,2)}%')
