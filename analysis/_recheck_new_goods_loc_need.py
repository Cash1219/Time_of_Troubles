...
for p in (root/'common'/'production_methods').glob('*.txt'):
    ...
    for mm in re.finditer(r'goods_(input|output)_([a-zA-Z0-9_]+)_add\s*=', t):
        mods.add(f'goods_{mm.group(1)}_{mm.group(2)}_add')
...
missing_goods=[g for g in new_goods if g not in loc_keys]
missing=[]
for g in new_goods:
    for io in ('input','output'):
        k=f'goods_{io}_{g}_add'; kd=f'{k}_desc'
        if k in mods and k not in loc_keys: missing.append(k)
        if k in mods and kd not in loc_keys: missing.append(kd)
print('missing_goods_name',missing_goods)
print('missing_new_goods_modifier_loc',missing)
